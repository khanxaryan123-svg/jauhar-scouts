...
export function Footer() {
  return (
    <footer id="journey" className="relative">
      ...
      <p className="pointer-events-none absolute bottom-[10%] right-[11%] whitespace-nowrap text-[clamp(12px,0.55vw,10px)] font-medium text-white">
      ...
      <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-end pb-16">
        {/* Main Footer Content */}
        <div className="pointer-events-auto w-[96%] xl:w-[92%] flex flex-col gap-2 xl:flex-row xl:items-center xl:justify-start xl:gap-10 -mt-8">
          ...
          <div className="flex shrink-0 items-center gap-3 -mt-2">
            ...
            <div className="flex items-center gap-2">
              {socialLinks.map(({ href, icon, alt }) => (
                <a key={alt} href={href} target="_blank" rel="noopener noreferrer" className="h-5 w-5 cursor-pointer transition hover:opacity-80">
          ...
          <div className="flex flex-1 translate-y-[0.6vw] flex-wrap items-start justify-center gap-x-4 gap-y-1 xl:justify-start xl:gap-x-8">
            {footerLinkGroups.map((group) => (
              <div key={group.title} className="flex min-w-[80px] flex-col gap-1.5">
                <Link href="#" className="text-[clamp(10px,0.8vw,16px)] font-bold uppercase tracking-wider text-white hover:text-white/90">{group.title}</Link>
                {group.links.map((link) => (
                  <Link key={link} href="#" className="cursor-pointer text-[clamp(9px,0.72vw,14px)] text-white/75 transition hover:text-white">{link}</Link>
                ))}
              </div>
            ))}
          </div>
